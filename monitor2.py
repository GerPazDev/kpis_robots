...
def render_edge_tab(df_pos: pd.DataFrame, tcol: str):
    ...
    for rid in robots:
        ...
        with st.expander(f"🤖 {rid}  —  {n_trades} trades totales", expanded=(len(robots) == 1)):
            ...
            def render_blocks_table(tab, blocks, scores, baseline_exp_val):
                with tab:
                    if not blocks:
                    ...
                    for i, (b, s) in enumerate(zip(blocks, scores)):
                        if prev_score is None:
                            ...
                            d = s - prev_score
                            delta = f"{'↑' if d > 0 else '↓' if d < 0 else '→'} {d:+d}"
                        rows.append({
                            "Bloque": f"#{b['bloque']}",
                    ...
                    st.dataframe(
                        df_blocks.style.applymap(color_score, subset=["Score"]),
            ...
            if mp_now is not None and mp_now <= -2:
                alerts.append(f"🛑 **Medio plazo en decay severo** (score {mp_now}): considerar pausar sistema")
            if lp_now is not None and lp_now <= -2:
                alerts.append(f"🛑 **Largo plazo en decay severo** (score {lp_now}): apagar y rediseñar")

            # Detectar tendencia bajista en 3 períodos consecutivos
            if len(lp_scores) >= 3 and all(lp_scores[-3+i] > lp_scores[-3+i+1] for i in range(2)):
                alerts.append("📉 **3 bloques LP consecutivos en baja** — decay estructural confirmado")
